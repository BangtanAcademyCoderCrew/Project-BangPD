...
const {
  batchItems,
  fetchAllMessagesByChannelSince,
  getGuildIdsWithoutBAE,
  sendGiveApplesEmbed,
} = require("../common/discordutil");
...
const getLogbookMessagesToCheck = async (messages) => {
  ...
  return messages.filter((m) => {
    const hasAlreadyBeenChecked =
      m.reactions.cache.has("👍") && m.reactions.cache.get("👍").me;

    // i.e. kckc class
    const ignoreMessageReaction = m.reactions.cache.find((r) =>
      r.emoji.name.includes(ignoreReactionName)
    );
...
const runGiveApplesJob = async (client, environment, guildIds) => {
  ...
  const getAllLogbookChannels = () => {
    ...
    if (environment === "dev") {
      ...
      return clientChannels.filter((c) =>
        c.name.toLowerCase().includes(logbookChannelNameStem)
      );
    }
    return guildIds.map((id) => {
      const guild = client.guilds.cache.get(id);
      const clientChannels = guild.channels.cache;
      const channelWithStem = clientChannels.find((c) =>
        c.name.toLowerCase().includes(logbookChannelNameStem)
      );
      if (channelWithStem) {
  ...
  const setLogbookUserIds = (message, userIds) => {
    const usersInMessage = Array.from(
      message.client.users.cache.filter((u) => userIds.includes(u.id)).values()
    );
    const usersInMessageIds = usersInMessage.map((user) => user.id);
    const messageUserIds = userIds.filter((uId) =>
      usersInMessageIds.includes(uId)
    );
  ...
  const setApplesToGive = async (channel) => {
    const messages = await fetchAllMessagesByChannelSince(
      channel,
      messagesSinceDateTime
    );
    if (messages.size === 0) {
      console.log(
        `Scheduled Job: giveApples - No logbooks were posted yesterday in guild: ${channel.guild.name} channel: ${channel.id}`
      );
      return;
    }
    ...
    if (filteredMessages.size === 0) {
      console.log(
        `Scheduled Job: giveApples - All logbooks posted yesterday in guild: ${channel.guild.name} channel: ${channel.id} have already been checked`
      );
      return;
    }
  ...
  const removeRollCallRole = async (member) => {
    ...
    const hasGreenAppleRole =
      member.roles.cache.has(greenAppleRoleId) ||
      usersWithGreenAppleAdded.has(member.id);
  ...
  const addActiveStudentRole = async (member) => {
    ...
    const hasRedAppleRole =
      member.roles.cache.has(redAppleRoleId) ||
      usersWithRedAppleAdded.has(member.id);
  ...
  const addAppleRoles = async () => {
    ...
    for (let i = 0; i < logbookUserIds.length; i++) {
      userFrequency[logbookUserIds[i]] = userFrequency[logbookUserIds[i]]
        ? userFrequency[logbookUserIds[i]] + 1
        : 1;
    }

    await Promise.all(
      membersNeedingApples.map(async (member) => {
        if (
          member.roles.cache.has(redAppleRoleId) &&
          member.roles.cache.has(greenAppleRoleId)
        ) {
  ...
  if (!resultsChannel) {
    console.log(
      `Scheduled Job: giveApples - No results channel with id ${logChannelId} found to send updates to.`
    );
    return;
  }
  ...
  if (!guildWithApples) {
    console.log(
      `Scheduled Job: giveApples - No guild was found for guildId ${guildId}.`
    );
    return sendErrorEmbed(
      resultsChannel,
      `No guild was found for guildId ${guildId}. 🙁`
    );
  }
  ...
  if (logbookChannels && logbookChannels.size === 0) {
    ...
    if (resultsChannel) {
      return sendErrorEmbed(
        resultsChannel,
        "No logbook channels were found. 🙁"
      );
    }
  }
  ...
  if (resultsChannel) {
    const description =
      "I've started checking yesterday's logbooks. Apples will be awarded shortly.";
    const fields = [
      {
        name: "From",
        value: `${messagesSinceDateTime.toLocaleString(
      ...
      {
        name: "Until",
        value: `${currentDateTimeCT.toLocaleString(DateTime.DATETIME_FULL)}`,
        inline: true,
      },
    ];
    await sendGiveApplesEmbed(
      resultsChannel,
      "🏁 Started!",
      description,
      fields
  ...
  await Promise.all(
    logbookChannels.map(async (channel) => {
      await sendLogEmbed(
        resultsChannel,
        `Checking yesterday's logbooks in ${channel}`,
  ...
  const greenAppleRole =
    guildWithApples.roles.cache.get(greenAppleRoleId) || "🍏";
  if (resultsChannel && usersWithGreenAppleAdded.size > 0) {
    await sendRolesChanged(
      resultsChannel,
      usersWithGreenAppleAdded,
      "🍏",
      "greenApple"
    );
  } else {
    await sendLogEmbed(
      resultsChannel,
      `No students were given the ${greenAppleRole} role.`
    );
  }
  ...
  if (resultsChannel && usersWithRedAppleAdded.size > 0) {
    ...
    await sendLogEmbed(
      resultsChannel,
      `No students were given the ${redAppleRole} role.`
    );
  }
  ...
  const rollCallRole =
    guildWithApples.roles.cache.get(rollCallRoleId) || "🔔 Roll Call";
  if (resultsChannel && usersWithRollCallRemoved.size > 0) {
    await sendRolesChanged(
      resultsChannel,
      usersWithRollCallRemoved,
      "🔔 Roll Call",
      "rollCall"
    );
  } else {
    await sendLogEmbed(
      resultsChannel,
      `No students had the ${rollCallRole} role removed.`
    );
  }

  // log students who had active student role added
  const activeStudentRole =
    guildWithApples.roles.cache.get(activeStudentRoleId) || "Active Student";
  if (resultsChannel && usersWithActiveStudentAdded.size > 0) {
    await sendRolesChanged(
      resultsChannel,
      usersWithActiveStudentAdded,
      "Active Student",
      "activeStudent"
    );
  } else {
    await sendLogEmbed(
      resultsChannel,
      `No students were given the ${activeStudentRole} role.`
    );
  }
  ...
  if (messagesWithApplesApplied.length > 0) {
    await sendLogEmbed(
      resultsChannel,
      "Marking checked logbooks as completed. 👍"
    );
    await Promise.all(
      messagesWithApplesApplied.map(async (msg) => {
        await msg.react("👍");
      })
    );
  }
  ...
  const formattedRuntime = Duration.fromObject({
    milliseconds: totalTimeElapsedInMs,
  }).toFormat("hh:mm:ss");
  const formattedRolesApplied =
    `${greenAppleRole} added: ${usersWithGreenAppleAdded.size}` +
    "\n" +
  ...
  const completedDescription =
    "All logbooks have been checked. See you next time!";
  const fields = [
...
module.exports = {
  ...
  async start(client, schedule, serverId) {
    ...
    if (serverId === "dev") {
      ...
      jobSchedule = schedule || cronSchedule;
    } else {
