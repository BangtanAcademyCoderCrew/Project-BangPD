...
module.exports = {
  bookmark(message, user) {
    ...
    if (!image && !message.content) {
      if (message.embeds[0]) {
        ...
        user
          .send(`Sent by: ${message.author.username}`, { embed })
          .then((msg) => msg.react("❌"));
        return;
      }
    }
  ...
  createBasicEmbed(name) {
    ...
    return new MessageEmbed().setColor(accentColor).setAuthor(author);
  },
  ...
  createPendingEmbed(username) {
    return this.createBasicEmbed().setDescription(
      `I am going over the books for you ${username}, please wait. :eyes:`
    );
  },

  async sendGiveApplesEmbed(
    channel,
    title,
    description,
    fields = [],
  ...
  createWordSearchEmbed(language, query, username, isDM, searchResults) {
    const embed = this.createBasicEmbed().setDescription(
    ...
    if (searchResults.length === 0) {
      ...
      this.setEmbedFooter(
        embed,
        `${username} can toggle languages. ${
          !isDM ? "Anyone can bookmark this message." : ""
        }`
      );
        ...
        if (language === "en") {
          embed.addField(
            `**${entry.word}**${entry.hanja ? ` (${entry.hanja})` : ""} - ${
              entry.wordTypeTranslated
            }${entry.pronunciation ? ` - [${entry.pronunciation}]` : ""}${
              entry.stars > 0 ? `  ${"★".repeat(entry.stars)}` : ""
            }`,
            defs.join("\n")
          );
        } else if (language === "ko") {
  ...
  createHanjaEmbeds(query, username, isDM, results) {
    const pages = [];
    const isEmpty =
      results.similarwords.length === 0 && results.hanjas.length === 0;
    if (isEmpty) {
      const embed = this.createBasicEmbed().setDescription(
        `Search results for: **${query}**`
      );
      ...
      while (results.hanjas.length > 0 || results.similarwords.length > 0) {
        const page = this.createBasicEmbed().setDescription(
          `Search results for: **${query}**`
        );
        let i;
        const hanjas = [];
  ...
  createExampleResultEmbed(language, query, username, isDM, searchResults) {
    const embed = this.createBasicEmbed().setDescription(
      `Example Sentences for for: **${query}**`
    );
    if (searchResults.length === 0) {
      embed.addField("Error", "No results have been found");
    } else {
      ...
      for (i = 0; i < searchResults.length; i += 1) {
        s += `**${i + 1}.** ${searchResults[i].example.replace(
          query,
          `**__${query}__**`
        )}\r\n\r\n`;
      }
  ...
  createHelpEmbed(commands) {
    const embed = this.createBasicEmbed("BangPD").setDescription(
      `Use **${prefix}dictionary-help <command>** to see information about a specific command.`
    );
    commands.forEach((c) => {
      if (c.name === "help") return;
      if (c.devOnly) return;
      const { name, aliases, description, examples } = c;

      const descriptionAndExamples =
        description + (examples ? `\r\n __(Ex. ${examples})__` : "");
      const title = `${prefix}${name} ${
        aliases ? `(short: ${aliases.map((e) => prefix + e).join(", ")})` : ""
      }`;
  ...
  createDetailHelpEmbed(command) {
    const embed = this.createBasicEmbed().setDescription(
      `**${prefix}${command.name} ${
        command.aliases
          ? `(short: ${command.aliases.map((e) => prefix + e).join(", ")})`
          : ""
    ...
    if (command.examples) {
      embed.addField(
        "Usage Example",
        command.examples ? command.examples : "None",
        true
      );
    }
    return embed;
    ...
    (async () => {
      ...
      try {
        ...
        usernames.forEach((username) => {
          const member = module.exports.getMemberByUsername(
            interaction,
            username.trim()
          );
          if (!member) {
      ...
      userIds.map(async (id) => {
        ...
        await Promise.all(
          guilds.map(async (guild) => {
            await guild.members.kick(id).catch((error) => {
              console.log(error);
              errorCount += 1;
  ...
  async banUsersOnServers(userIds, guilds) {
    ...
    await Promise.all(
      userIds.map(async (id) => {
        ...
        await Promise.all(
          guilds.map(async (guild) => {
            await guild.members.ban(id).catch((error) => {
              console.log(error);
              errorCount += 1;
              errorUsersPerServer[guild.id] += `<@${id}>\n`;
            });
  ...
  async fetchAllMessagesByChannelSince(channel, sinceDateTime) {
    ...
    while (message) {
      await channel.messages
        .fetch({ limit: 100, before: message.id })
        .then((messagePage) => {
          const filteredByDate = messagePage.filter(
            (m) => m.createdTimestamp >= sinceDateTime
          );
          messages = messages.concat(filteredByDate);
          // Update message pointer to be last message in page of messages
          message =
            filteredByDate.size > 0
  ...
  getUserTimezoneRole(member) {
    ...
    return member.roles.cache
      .find((r) => r.name.toLowerCase().includes(timezoneRole))
      .name.toLowerCase();
  },
  ...
  getUserLevelRole(member) {
    ...
    return member.roles.cache
      .find((r) => r.name.toLowerCase().includes(levelRoleName))
      .name.toLowerCase();
  },
  ...
  getUserPronounsRoles(member) {
    ...
    return member.roles.cache
      .filter((r) => pronounsRoles.includes(r.name.toLowerCase()))
  ...
  assignRolesFromMainServer(guild, memberOnBA, memberInNewGuild) {
    const passportRoles = this.getUserPassportRoles(memberOnBA);

    const rolesToAddIds = guild.roles.cache
      .filter((r) => passportRoles.includes(r.name.toLowerCase()))
      .map((r) => r.id);
    memberInNewGuild.roles.add(rolesToAddIds);
  },

  updatePassportRolesInSatelliteServer(
    newGuildId,
    newMember,
    roleType,
    newRole,
    ...
    const guildMember = newGuild.members.cache.find(
      (u) => u.id === newMember.id
    );
    if (!guildMember) {
      return;
    ...
    if (roleType != "pronouns") {
      oldRole = guildMember.roles.cache.find((r) =>
        r.name.toLowerCase().includes(roleType)
      );

      if (!oldRole) {
        console.log(
          `User ${guildMember.displayName} doesn't have a ${roleType} role`
        );
        return;
      }
    }

    const newRoleInGuild = newGuild.roles.cache.find(
      (r) => r.name.toLowerCase() === newRole
    );
  ...
  removePronounsRoleInSatelliteServer(
    newGuildId,
    newMember,
    removedRole,
    client
    ...
    const roleToRemove = newGuild.roles.cache.find(
      (r) => r.name.toLowerCase() === removedRole
    );
