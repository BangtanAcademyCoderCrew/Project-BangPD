...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Gets all users with the ROLLCALL role and kicks them from all servers."
    )
  ...
  async execute(interaction) {
    ...
    const getUsersWithRollCall = () => {
      const ids = Array.from(
        guilds[0].roles.cache.get(rollcallTagId).members.keys()
      );
    ...
    if (userIds.length > 0) {
      await interaction.followUp({
        content:
          "Are you sure you want to kick these users from all BA servers?",
      ...
      collector.on("collect", async (click) => {
        if (click.customId === `confirmKick_${interactionId}`) {
          ...
          await interaction.followUp({
            content:
              "These ROLLCALL users were kicked from at least one server",
            files: [attachment],
            ephemeral: true,
          });
      ...
      return interaction.followUp({
        content: "There are no users with ROLLCALL tag",
        ephemeral: true,
      });
    }
  },
