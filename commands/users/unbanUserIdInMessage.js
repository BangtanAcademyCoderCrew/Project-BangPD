...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Gets a list of user ids from a message(s) and unbans them from all servers."
    )
  ...
  async execute(interaction) {
      ...
      channel.messages
        ...
        .then(async (msg) => {
          ...
          for (const id of userIds) {
            ...
            await Promise.all(
              guilds.map(async (guild) => {
                await guild.members.unban(id).catch((error) => {
                  console.log(error);
                  errorCount += 1;
          ...
          const attachment = new MessageAttachment(
            Buffer.from(membersUnbanned, "utf-8"),
            "usersID.txt"
          );
          interaction.followUp({
            content: `These users in message ${messageId} were unbanned from at least one server`,
            files: [attachment],
            ephemeral: true,
          });
        })
    ...
    await interaction.followUp({
      content:
        "Are you sure you want to unban these users from all BA servers?",
          ...
          Object.entries(errorUsersPerServer).forEach(([key, value]) => {
            if (value !== "") {
              ...
              return interaction.followUp({
                content: `There was an error unbanning these users from server with ID ${key} <a:shookysad:949689086665437184>`,
