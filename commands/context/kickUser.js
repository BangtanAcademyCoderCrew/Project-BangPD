...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await interaction.reply({
      content: `Are you sure you want to kick user ${user} from all BA servers?`,
    ...
    collector.on("collect", async (click) => {
      if (click.customId === `confirmKick_${interactionId}`) {
        ...
        const [membersKicked, errorUsersPerServer] =
          await DiscordUtil.kickUsersOnServers([user.id], guilds);
        let errorGuilds = "";
        ...
        return interaction.followUp({
          content: `User ${user} could not be kicked from servers with IDs ${errorGuilds}`,
          ephemeral: true,
        });
      }
      if (click.customId === `cancelKick_${interactionId}`) {
        collector.stop();
