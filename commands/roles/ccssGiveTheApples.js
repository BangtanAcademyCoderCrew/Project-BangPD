...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Assigns a role to users mentioned in a message. If they have the 1st role, the 2nd role is assigned"
    )
    ...
    .addStringOption((option) =>
      option
        .setName("first_role_id")
        .setDescription("What role would you like to add to user?")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("second_role_id")
        .setDescription(
          "What role would you like to add to user if they have the first role?"
        )
        .setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    if (serverId && !guild) {
      return interaction.followUp({
        content: `I can't find server with ID ${serverId} :pensive:`,
      });
    }
    ...
    if (!guildChannel) {
      return interaction.followUp({
        content: `I can't find channel with ID ${channelId} in server ${guild.name} :pensive:`,
      });
    }
      ...
      guildChannel.messages
        ...
        .then((msg) => {
          if (
            ...
            msg.reactions.cache.get("👍").me
          ) {
            return interaction.followUp(
              "You already checked this message before!"
            );
          }
          ...
          for (let i = 0; i < ids.length; i++) {
            userFrequency[ids[i]] = userFrequency[ids[i]]
              ? userFrequency[ids[i]] + 1
              : 1;
          }
          ...
          members.forEach((member) => {
            ...
            if (
              userFrequency[member.id] >= 2 &&
              usersWithFirstRole.indexOf(member.id) === -1
            ) {
          ...
          const attachmentFirstRole = new MessageAttachment(
            Buffer.from(usersWithFirstRole, "utf-8"),
            "usersID-firstRole.txt"
          );
          const attachmentSecondRole = new MessageAttachment(
            Buffer.from(usersWithSecondRole, "utf-8"),
            "usersID-secondRole.txt"
          );

          // Creates embeds with userIds and sends them

          const firstRoleEmbed = DiscordUtil.createApplesAssignEmbed(
            "Green Apple",
            usersWithFirstRole
          );
          const secondRoleEmbed = DiscordUtil.createApplesAssignEmbed(
            "Red Apple",
            usersWithSecondRole
          );

          interaction.followUp({
            content: `Users in message ${messageId} added role ${firstRole} and ${secondRole}`,
            files: [attachmentFirstRole, attachmentSecondRole],
            embeds: [firstRoleEmbed, secondRoleEmbed],
          });
          msg.react("👍");
        })
        .catch((error) => {
          console.error(error);
          interaction.followUp(
            `Message with ID ${messageId} wasn't found in channel <#${channelId}> <a:shookysad:949689086665437184>`
          );
        });
    };
    ...
    allMessageIDs.forEach((messageId) =>
      assignRoles(messageId, firstRoleToAssignId, secondRoleToAssignId)
    );
  },
};
