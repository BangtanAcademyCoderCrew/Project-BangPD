...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("rollcall")
    .setDescription(
      "Starts rollcall. You will need the role ids that won't be rollcalled and the rollcall role id."
    )
    .addStringOption((option) =>
      option
        .setName("role_exception_ids")
        .setDescription(
          "What roles are needed to not be rollcalled (ex. Active Student, level 4, etc)?"
        )
        .setRequired(true)
    )
    .addRoleOption((option) =>
      option
        .setName("rollcall_role_id")
  ...
  async execute(interaction) {
    ...
    return interaction.followUp({
      content: `Rollcall done. ${rollcalled} are in roll call. ${activeMembers} active members.`,
    });
  },
};
