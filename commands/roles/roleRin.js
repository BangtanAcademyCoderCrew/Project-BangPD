...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addRoleOption((option) =>
      option
        .setName("assigned_role")
        .setDescription(
          "What role would you like to remove from users (assigned role)?"
        )
        .setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    const members = interaction.guild.members.cache.filter((member) =>
      member.roles.cache.has(baseRole.id)
    );
    ...
    const attachment = new MessageAttachment(
      Buffer.from(usersWithRolesRemoved, "utf-8"),
      "usersID.txt"
    );
    return interaction.followUp({
      content: `Users with role ${baseRole} removed role ${assignedRole}`,
      files: [attachment],
    });
  },
};
