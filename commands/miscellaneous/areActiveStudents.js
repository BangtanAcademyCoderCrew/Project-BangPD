...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Gets a list of user ids that were mentioned in a message and see if they are active or not."
    )
    ...
    const checkIDs = (messageID) => {
      channel.messages
        ...
        .then((msg) => {
          ...
          const notActiveStudents = ids.filter(
            (id) => !activeStudents.includes(id)
          );
          const attachmentActive = new MessageAttachment(
            Buffer.from(`<@${activeStudents.join(">\n<@")}>`, "utf-8"),
            "activeStudents.txt"
          );
          const attachmentNotActive = new MessageAttachment(
            Buffer.from(`<@${notActiveStudents.join(">\n<@")}>`, "utf-8"),
            "notActiveStudents.txt"
          );
          interaction.followUp({
            content: `Users in message ${messageID} who are active`,
            files: [attachmentActive],
          });
          interaction.followUp({
            content: `Users in message ${messageID} who are not active`,
            files: [attachmentNotActive],
          });
        })
        .catch((error) => {
          console.error(error);
