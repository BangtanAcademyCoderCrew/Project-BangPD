...
module.exports = {
    ...
    async execute(interaction) {
        ...
        channel.messages.fetch(messageId).then((message) => {
            ...
            if(attachment){
                return interaction.reply({ content: `Message with ID ${messageId} has file ${attachment.url}` })
            } 
            return interaction.reply({ content: `Message with ID ${messageId} doesn't have a file` })
        }).catch((error) => {
            console.log(error);
