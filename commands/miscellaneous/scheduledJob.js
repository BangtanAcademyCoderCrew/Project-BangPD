...
module.exports = {
      ...
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName("schedule")
            .setDescription(
              'The cron schedule to override the default schedule. Default is "0 2 * * *" (daily at 2am)'
            )
            .setRequired(false)
        )
        .addStringOption((option) =>
          option
            .setName("server")
            .setDescription(
              "The server to run the scheduled job on and override default. Default is all BA servers"
            )
            .setRequired(false)
  ...
  async execute(interaction) {
    ...
    if (name) {
      ...
      const scheduledJobFiles = fs
        .readdirSync(scheduledJobsDirectory)
        .filter((file) => file.endsWith(".js"));
      const jobFile = scheduledJobFiles.find((file) => file.includes(name));
      if (!jobFile) {
        return interaction.reply({
          content: `There was no scheduled job with name "${name}" found. 🙁`,
        });
      }
    }
    ...
    if (schedule) {
      ...
      if (!isValidSchedule) {
        return interaction.reply({
          content: `${schedule} is not a valid cron schedule. 🙁`,
        });
      }
    }
    ...
    if (commandName === "start") {
      ...
      if (isSuccessful) {
        return interaction.reply({
          content: `Scheduled Job with name "${name}" has started. 🟢`,
        });
      }
      return interaction.reply({
        content: `Scheduled Job with name "${name}" could not be started. 🙁`,
      });
    }
    ...
    if (commandName === "stop") {
      ...
      if (isSuccessful) {
        return interaction.reply({
          content: `Scheduled Job with name "${name}" has stopped. 🔴`,
        });
      }
      return interaction.reply({
        content: `Scheduled Job with name "${name}" could not be stopped. 🙁`,
      });
    }
