...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Gets a list of user nicknames and names from a csv/txt file with list of all the user ids')
    .addStringOption(option =>
      option.setName('file_url')
        .setDescription('The url of the csv/txt with list of user ids, one per line')
        .setRequired(true))
  ...
  async execute(interaction) {
    ...
    const attachment = new Discord.MessageAttachment(Buffer.from(`${userNicknamesAndTags.join('\n')}`, 'utf-8'), 'userNames.txt');
    interaction.followUp({ content: 'User(s) nicknames and tags', files: [attachment] });
  }
};
