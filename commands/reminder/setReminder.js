...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("time_in_advance")
        .setDescription(
          "The time before the deadline you would like the reminder to set off"
        )
  ...
  async execute(interaction) {
    ...
    const reminderHourBeforeMessage = `Reminder will trigger at (CST): ${reminderTimeCST.toLocaleString(
      DateTime.DATETIME_SHORT
    )}`;
    const reminderPromise = `I will send reminder ${reminderTimeCST.toRelative()} in channel : ${
      channel.name
    }`;
    const newLine = "\n";
